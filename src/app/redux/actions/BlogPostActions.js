import { GET_BLOG_POST_LIST, GET_BLOG_POST_LIST_BY_CATEGORY, ADD_BLOG_POST, DELETE_BLOG_POST, PUT_BLOG_POST } from "./actionNames";
...
export const addBlogPost = (payload) => (dispatch) => {
  ...
  axios.post(url, payload )
    ...
    .then(() => messages_container.innerHTML = `A blog post with title: <b>${payload.title}</b> has been added.`)
...
export const getBlogPostListByCategory = (categoryId) => dispatch => {
  ...
  axios.get(`${url}/GetPostByCategory?categoryId=${categoryId}`)
    ...
    .then(() => messages_container.innerHTML = `You have listed all the posts with: <b>${categoryId}</b> categoryId`)
    .catch( err => console.log(err));
};
...
export const deleteBlogPost = (id) => dispatch => {
  ...
  axios({
    ...
    .then(() => messages_container.innerHTML = `A blog post with id: <b>${id}</b> has been deleted.`)
    .catch( err => console.log(err));
};
...
export const putBlogPost = (payload) => dispatch => {
  ...
  axios({
    ...
    .then(() => messages_container.innerHTML = `A blog post with title: <b>${payload.title}</b> has been updated.`)
    .catch( err => console.log(err))
};